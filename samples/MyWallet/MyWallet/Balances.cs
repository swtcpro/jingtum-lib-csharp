...
namespace MyWallet
{
    public class Balances
    {
        ...
        private static Balance[] ProcessBalance(Results data, Amount condition)
        {
            ...
            var freeze0 = Global.FreezedReserved + freezedCount * Global.FreezedEachFreezed;
            var swt = new Balance { Currency = Global.Currency, Issuer = "", Value = swtValue, Freezed = freeze0.ToString(Global.AmountFormat) };

            var balances = new List<Balance>();
            if((condition.Currency==null && condition.Issuer==null) || condition.Currency == Global.Currency){
                balances.Add(swt);
            }

            if (condition.Currency == Global.Currency)
            {
